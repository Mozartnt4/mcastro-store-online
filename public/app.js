...
function renderCash(){const ins=db.cash.filter(c=>c.type==='in').reduce((s,c)=>s+c.value,0),outs=db.cash.filter(c=>c.type==='out').reduce((s,c)=>s+c.value,0);$('cashIn').textContent=money(ins);$('cashOut').textContent=money(outs);$('cashBalance').textContent=money(ins-outs);$('replacement').textContent=money(db.sales.reduce((s,v)=>s+v.cost,0));$('cashList').innerHTML=db.cash.length?db.cash.map(c=>`<div class="listRow"><div><b>${esc(c.description)}</b><small>${new Date(c.date).toLocaleString('pt-BR')} · ${esc(c.method)}</small></div><strong style="color:${c.type==='in'?'#059669':'#dc2626'}">${c.type==='in'?'+':'-'} ${money(c.value)}</strong></div>`).join(''):'<div class="empty">Nenhuma movimentação.</div>'}
function renderDashboard(){const revenue=db.sales.reduce((s,v)=>s+v.total,0),profit=db.sales.reduce((s,v)=>s+v.profit,0),stock=db.products.reduce((s,p)=>s+p.cost*p.stock,0),cash=db.cash.reduce((s,c)=>s+(c.type==='in'?c.value:-c.value),0),low=db.products.filter(p=>p.stock<=p.min);$('heroRevenue').textContent=money(revenue);$('statProducts').textContent=db.products.length;$('statLow').textContent=low.length+' alertas';$('statStock').textContent=money(stock);$('statProfit').textContent=money(profit);$('statCash').textContent=money(cash);$('lowList').innerHTML=low.length?low.slice(0,6).map(p=>`<div class="listRow"><div><b>${esc(p.name)}</b><small>${esc(p.sku)}</small></div>${stockBadge(p)}</div>`).join(''):'<div class="empty">Estoque sob controle.</div>';$('recentSales').innerHTML=db.sales.length?db.sales.slice(0,6).map(s=>`<div class="listRow"><div><b>Venda ${s.id.slice(-6).toUpperCase()}</b><small>${new Date(s.date).toLocaleString('pt-BR')}</small></div><strong>${money(s.total)}</strong></div>`).join(''):'<div class="empty">Nenhuma venda.</div>'}
