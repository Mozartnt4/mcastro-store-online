...
function renderDashboard(){const completed=db.sales.filter(s=>s.status==='concluido'||!s.status),revenue=completed.reduce((s,v)=>s+v.total,0),profit=completed.reduce((s,v)=>s+v.profit,0),stock=db.products.reduce((s,p)=>s+p.cost*p.stock,0),cash=db.cash.reduce((s,c)=>s+(c.type==='in'?c.value:-c.value),0),low=db.products.filter(p=>p.stock<=p.min);$('heroRevenue').textContent=money(revenue);$('statProducts').textContent=db.products.length;$('statLow').textContent=low.length+' alertas';$('statStock').textContent=money(stock);$('statProfit').textContent=money(profit);$('statCash').textContent=money(cash);$('lowList').innerHTML=low.length?low.slice(0,6).map(p=>`<div class="listRow"><div><b>${esc(p.name)}</b><small>${esc(p.sku)}</small></div>${stockBadge(p)}</div>`).join(''):'<div class="empty">Estoque sob controle.</div>';$('recentSales').innerHTML=db.sales.length?db.sales.slice(0,10).map(s=>{const pending=s.status==='aguardando_comprovante',canceled=s.status==='cancelado',status=pending?'Aguardando comprovante':canceled?'Cancelado':'Concluída';return `<div class="listRow"><div><b>Pedido ${esc(s.id)}</b><small>${esc(s.customerName||'Cliente')} · ${new Date(s.date).toLocaleString('pt-BR')} · ${status}</small></div><div><strong>${money(s.total)}</strong>${pending?`<div class="orderActions"><button class="primary" onclick="completeSale('${esc(s.id)}')">Venda concluída</button><button class="secondary" onclick="cancelSale('${esc(s.id)}')">Cancelar pedido</button></div>`:''}</div></div>`}).join(''):'<div class="empty">Nenhuma venda.</div>'}
