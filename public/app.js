...
window.cancelSale=window.cancelSale||function(id){const sale=db.sales.find(s=>s.id===id);if(!sale||sale.status!=='aguardando_comprovante')return;if(!confirm('Cancelar este pedido?'))return;sale.status='cancelado';save();toast('Pedido cancelado')};
window.refundSale=window.refundSale||function(id){const sale=db.sales.find(s=>s.id===id);if(!sale||sale.status!=='concluido')return;const choice=prompt('Digite 1 para devolver ao estoque ou 2 para produto com defeito.');if(choice!=='1'&&choice!=='2')return;const back=choice==='1';if(back)sale.items.forEach(item=>{const p=db.products.find(x=>x.id===item.id);if(p)p.stock+=item.qty});sale.status=back?'estornado_estoque':'estornado_defeito';db.cash.unshift({id:uid(),date:new Date().toISOString(),type:'out',description:'Estorno '+sale.id,value:sale.total,method:sale.payment,saleId:sale.id});save();toast('Venda estornada')};
function renderDashboard(){const completed=db.sales.filter(s=>s.status==='concluido'||!s.status),revenue=completed.reduce((s,v)=>s+v.total,0),profit=completed.reduce((s,v)=>s+v.profit,0),stock=db.products.reduce((s,p)=>s+p.cost*p.stock,0),cash=db.cash.reduce((s,c)=>s+(c.type==='in'?c.value:-c.value),0),low=db.products.filter(p=>p.stock<=p.min);$('heroRevenue').textContent=money(revenue);$('statProducts').textContent=db.products.length;$('statLow').textContent=low.length+' alertas';$('statStock').textContent=money(stock);$('statProfit').textContent=money(profit);$('statCash').textContent=money(cash);$('lowList').innerHTML=low.length?low.slice(0,6).map(p=>`<div class="listRow"><div><b>${esc(p.name)}</b><small>${esc(p.sku)}</small></div>${stockBadge(p)}</div>`).join(''):'<div class="empty">Estoque sob controle.</div>';$('recentSales').innerHTML=db.sales.length?db.sales.slice(0,10).map(s=>{const pending=s.status==='aguardando_comprovante',canceled=s.status==='cancelado',refunded=String(s.status||'').startsWith('estornado'),status=pending?'Aguardando comprovante':canceled?'Cancelado':s.status==='estornado_estoque'?'Estornado · voltou ao estoque':s.status==='estornado_defeito'?'Estornado · produto com defeito':'Concluída';return `<div class="listRow"><div><b>Pedido ${esc(s.id)}</b><small>${esc(s.customerName||'Cliente')} · ${new Date(s.date).toLocaleString('pt-BR')} · ${status}</small></div><div><strong class="${canceled||refunded?'mutedValue':''}">${money(s.total)}</strong>${pending?`<div class="orderActions"><button class="primary" onclick="completeSale('${esc(s.id)}')">Venda concluída</button><button class="secondary" onclick="cancelSale('${esc(s.id)}')">Cancelar pedido</button></div>`:s.status==='concluido'?`<div class="orderActions"><button class="secondary refundAction" onclick="refundSale('${esc(s.id)}')">Estornar venda</button></div>`:''}</div></div>`}).join(''):'<div class="empty">Nenhuma venda.</div>'}
$('saveSettings').onclick=()=>{db.settings={storeName:$('storeName').value.trim(),pixKey:$('pixKey').value.trim(),whatsapp:$('whatsapp').value.trim()};save();toast('Configurações salvas')};$('backup').onclick=()=>{const a=document.createElement('a');a.href=URL.createObjectURL(new Blob([JSON.stringify(db,null,2)],{type:'application/json'}));a.download='backup-mcastro-'+new Date().toISOString().slice(0,10)+'.json';a.click()};if($('restore'))$('restore').onchange=e=>{const f=e.target.files[0];if(!f)return;const r=new FileReader();r.onload=()=>{try{db=JSON.parse(r.result);save();toast('Backup restaurado')}catch(e){alert('Arquivo inválido')}};r.readAsText(f)};
